...
5. get_department_phone_number: 한라대학교 학과별 전화번호/연락처 조회
...
5. **학과 전화번호/연락처 관련** → **반드시 get_department_phone_number 선택**
   - 키워드: "학과 전화번호", "학과 연락처", "○○학과 전화", "○○학과 몇 번", "행정실 번호"
   - ⚠️ **절대 search_internet 사용 금지**: 학과 전화번호는 웹 검색이 아닌 get_department_phone_number 함수로만 조회
6. **웹 검색 필요**: 최신 뉴스, 날씨, 일반 상식, 학교 외부 정보만 → search_internet
   - ❌ 규정/통학버스/식당/학사일정/학과 전화번호는 웹 검색 금지
7. **함수 불필요**: 일반 대화, 간단한 질문 → 빈 배열 반환
8. **여러 함수 필요**: 복합 질문일 경우 필요한 모든 함수를 배열에 포함
...
```
{
  "reasoning": "사용자가 컴퓨터공학과의 전화번호를 묻고 있습니다. 학과 전화번호 조회는 get_department_phone_number 함수로 처리합니다. 절대 search_internet을 사용하지 않습니다.",
  "selected_tools": ["get_department_phone_number"]
}
```
