...
@router.post("")
async def submit_survey(request: SurveyRequest, mongo_client: AsyncIOMotorDatabase = Depends(get_mongo_db)):
    try:
        survey_data = request.model_dump()
        await save_to_mongodb(mongo_client, survey_data, "survey")
