...
중요: 현재 시간 컨텍스트를 참조하여 날짜를 정확하게 전달하세요.
- "오늘" → date="오늘" (그대로 전달)
- "어제" → date="어제" (그대로 전달)
- "내일" → date="내일" (그대로 전달)
- "이번 주 화요일" → 현재 날짜를 확인하여 해당 주의 화요일 날짜를 YYYY-MM-DD 형식으로 계산하여 전달
- "다음 주 월요일" → 현재 날짜를 확인하여 다음 주의 월요일 날짜를 YYYY-MM-DD 형식으로 계산하여 전달
- 요일만 언급 ("화요일", "수요일") → 이번 주의 해당 요일 날짜를 YYYY-MM-DD 형식으로 계산하여 전달

예시:
- 사용자: "어제 학식" → date="어제"
- 사용자: "오늘 점심" → date="오늘"
- 사용자: "이번 주 화요일 메뉴" → 현재가 2025-10-25(토)이면 → date="2025-10-21" (이번 주 화요일)
- 사용자: "화요일 학식" → 현재가 2025-10-25(토)이면 → date="2025-10-21" (이번 주 화요일)""",
            "parameters": {
                "type": "object",
                "required": ["date"],
                "properties": {
                    "date": {
                        "type": "string",
                        "description": """조회할 날짜. 
- 상대 날짜 표현: '오늘', '어제', '내일', '모레'는 그대로 전달
- 요일 표현: 이번 주의 해당 요일 날짜를 YYYY-MM-DD 형식으로 계산하여 전달
- 절대 날짜: YYYY-MM-DD 또는 YYYY.MM.DD 형식""",
                    },
                    "meal": {
                        "type": "string",
                        "enum": ["조식", "중식", "석식"],
...
class FunctionCalling:
    ...
    def analyze(self, user_message, tools):
        ...
        try:
            ...
            enhanced_system_prompt = f"""{character.decide_function}
