...
## 사용 가능한 함수
1. search_internet: 웹 검색 (최신 정보, 실시간 뉴스, 일반 지식)
2. get_halla_cafeteria_menu: 한라대학교 학생식당 메뉴 조회 (학식, 식단, 점심, 저녁 등)

## 판단 규칙
1. **학식/식단 관련**: "학식", "메뉴", "식단", "점심", "저녁" → get_halla_cafeteria_menu
2. **웹 검색 필요**: 최신 정보, 뉴스, 일반 지식, 공지사항 → search_internet
3. **함수 불필요**: 일반 대화, 간단한 질문 → 빈 배열 반환
4. **여러 함수 필요**: 복합 질문일 경우 필요한 모든 함수를 배열에 포함
